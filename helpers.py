...
def reboot():
    """
    Remotely Reboots the MicroController.
    ...
    x = conn.cursor()	
    try:
	    x.execute("UPDATE ardu_quick_command SET Reboot_Req = 1, Reboot_Clear = 0 LIMIT 1")
            conn.commit()
	    print _('Rebooting microcontroller')

    except Exception as e:
            print "Unable to initiate microcontroller reset (try performing manually/locally): " + str(e) 
...
def poweroff(wait=1, block=False):
    """
    Powers Off (Forces Micocontroller to stop all Watering Events)
...
def schedule_stations(stations):
    ...
    if gv.sd['seq']:  # sequential mode, stations run one after another
        for b in range(len(stations)):
			...
			DOW_and_UTC = day + str(accumulate_time)
			...
			try:
    				x.execute("UPDATE schedule SET DOW_Plus_UTC = %s, Duration = %d, Delay_Minutes = %d WHERE Zone_Num = %d and Rain_Delay_Allow = 0", (DOW_and_UTC, dur, accumulate_time, s))
        			conn.commit()
				print "Scheduler Updated"
			except Exception as e:
        			print "Unable to update schedule: " + str(e) 
...
def stop_onrain():
    ...
    if do_set_output:
    	x = conn.cursor()
    	try:
    		x.execute("UPDATE schedule SET Delay_Minutes = %d WHERE Rain_Delay_Allow = %d", (accumulate_time, 1))
        	conn.commit()
		print "Rain Delay Initiated" 

    	except Exception as e:
        	print "Unable to perform Rain Delay Request request: " + str(e) 
...
def stop_stations():
    ...
    try:
        ...
        conn.commit()
	print "Cancel Request initiated for entire schedule" 

    except Exception as e:
        print "Unable to initiate cancel request: " + str(e) 
